// PID: 730400003
// I pledge the COMP 211 honor code.

#include <ctype.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/types.h>
#include <sys/wait.h>
#include <sys/stat.h>
#include <unistd.h>

#include "shell.h"

const char *BUILT_IN_COMMANDS[] = {"cd", "exit", NULL};
const char *PATH_SEPARATOR = ":";

void parse(char *line, command_t *p_cmd){
    char *token;
    int argc = 0;
    
    while((token = strtok_r(line, " \n\t\r", &line)) != NULL){
        strcpy(p_cmd->argv[argc], token);
        argc++;
    }

    strcpy(p_cmd->path, p_cmd->argv[0]);
    p_cmd->argc = is_builtin(p_cmd) || find_fullpath(p_cmd) ? argc : ERROR;
} // end parse function

bool find_fullpath(command_t *p_cmd){
    // TODO: complete this function!

    // Path environmental variables. Looks something like:
    // /usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/mnt/learncli/bin
    char path[300];
    strcpy(path, getenv("PATH"));

    char *token = strtok(path, PATH_SEPARATOR);

    while(token != NULL){
        // Here, token is a directory on the path, such as "/usr/local/bin"
        // You'll want to use some functions in string.h to make a (potential) full path.
        // For example, for `ls`, `/usr/local/bin/ls` is a potential full path.
        // Then, you want to use the `stat` function to tell if it is the full path.

        // Debug:
        // printf("%s\n", token);

	//ALLOCATING SPACE FOR THE FINAL PATH NAME
	char *pathName = malloc(300 * sizeof(path) +1);

	//COPY TOKEN NAME TO PATH NAME
	strcpy(pathName, token);
	//ADD SLASH TO PATHNAME
	strcat(pathName,"/");
	//ADD THE USER COMMAND FROM THE STRUCT TO THE PATHNAME
	strcat(pathName,p_cmd->path);

	//RUN STAT ON PATHNAME, UPDATE PATH IN STRUCT AND RETURN TRUE IF IT EXISTS
	//STAT CODE FROM README:
	struct stat buffer;
	int exists;
	char *file_or_dir; // string representing the full path of a file/dir
	exists = stat(pathName, &buffer);
	if(exists == 0 && (S_IFREG & buffer.st_mode)){
		//SET THE STRUCT PATH TO THE PATHNAME
		strcpy(p_cmd -> path, pathName);
		  // File exists
		//p_cmd->path = token;
		return true;
	}

        token = strtok(NULL, PATH_SEPARATOR);
    }

    return false;
} // end find_fullpath function

int execute(command_t *p_cmd){
    // TODO: complete this function!
    int status = SUCCESSFUL;
    int child_process_status;
    pid_t child_pid;

    //FORK - SET PID TO FORK, CHILD BECOMES 0
    child_pid = fork();
    //EXEC - THE CHILD WILL EXECUTE THE COMMAND WITH EXECV AND THEN EXIT, SET STATUS TO ERROR IF EXECUTION FAILS
    if(child_pid == 0){
      // This is the child
      // Execute in same environment as parent
	p_cmd->argv[p_cmd->argc] = NULL;
        execv(p_cmd->path, p_cmd->argv);
        perror("Execute terminated with an error condition!\n");
	status = ERROR;
        exit(1);
    }
    //WAIT - THE PARENT WILL RUN WAIT
    child_process_status = wait(NULL);
    //RETURN THE ERROR STATUS IF CHILD RETURNS ERROR
    if(child_process_status == ERROR) {
    	status = ERROR;
    }

    return status;
} // end execute function


bool is_builtin(command_t *p_cmd){
    int i = 0;

    while(BUILT_IN_COMMANDS[i] != NULL){
        if(strcmp(p_cmd->path, BUILT_IN_COMMANDS[i]) == 0){
            return true;
        }

        i++;
    }

    return false;
} // end is_builtin function


int do_builtin(command_t *p_cmd){
    // For whatever reason, `exit` is handled in driver.c.
    // This just handles `cd`

    struct stat buff;
    int status = ERROR;

    if(p_cmd->argc == 1){

        // -----------------------
        // cd with no arg
        // -----------------------
        // change working directory to that
        // specified in HOME environmental 
        // variable

        status = chdir(getenv("HOME"));
    }else if((stat(p_cmd->argv[1], &buff) == 0 && (S_IFDIR & buff.st_mode))){
        // -----------------------
        // cd with one arg 
        // -----------------------
        // only perform this operation if the requested
        // folder exists

        status = chdir(p_cmd->argv[1]);
    }

    return status;
} // end do_builtin function

void cleanup(command_t *p_cmd){
    int i = 0;

    while(p_cmd->argv[i] != NULL){
        free(p_cmd->argv[i]);
        i++;
    }

    free(p_cmd->argv);
    free(p_cmd->path);	
} // end cleanup function
